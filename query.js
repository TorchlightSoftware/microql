  ...
  for (const serviceName of usedServices) {
    ...
    if (service && typeof service.tearDown === 'function') {
      try {
        ...
        if (settings?.debug) {
          console.error(
            `Warning: tearDown failed for service ${serviceName}:`,
            error.message
          )
        }
      }
...
async function loadSnapshot(snapshotFile, ast, debug) {
  if (!(await fs.pathExists(snapshotFile))) return
...
export default async function query(config) {
  ...
  try {
    ...
    if (snapshotFile)
      await loadSnapshot(snapshotFile, ast, resolvedSettings.debug)
    ...
    await callTearDownOnUsedServices(
      ast.usedServices,
      preparedServices,
      resolvedSettings
    )
    ...
    if (queryOnError && Array.isArray(queryOnError)) {
      try {
        ...
        console.error(
          `Query-level onError handler failed: ${onErrorErr.message}`
        )
      }
    }
