...
describe('TearDown Tests', () => {
  ...
  it('should call tearDown on multiple services', async () => {
    ...
    const service1 = {
      async action() { return 'result1' },
      async tearDown() { tearDownCalls.push('service1') }
    }
    
    const service2 = {
      async action() { return 'result2' },
      async tearDown() { tearDownCalls.push('service2') }
    }
    ...
    assert.strictEqual(tearDownCalls.length, 2, 'Both services should have tearDown called')
    assert(tearDownCalls.includes('service1'), 'service1 tearDown should be called')
    assert(tearDownCalls.includes('service2'), 'service2 tearDown should be called')
  })
  ...
  it('should call tearDown even if query fails', async () => {
    ...
    assert.strictEqual(tearDownCalled, true, 'tearDown should be called even when query fails')
  })
  ...
  it('should not call tearDown for unused services', async () => {
    ...
    const usedService = {
      async action() { return 'result' }
    }
    
    const unusedService = {
      async action() { return 'unused' },
    ...
    assert.strictEqual(tearDownCalled, false, 'tearDown should not be called for unused services')
  })
