...
describe('Debug Color Tests', () => {
  ...
  it('should use unique colors for different services in debug mode', async () => {
    await query({
      settings: { debug: true },
      services: {
        serviceA: { async test() { return 'resultA' } },
        serviceB: { async test() { return 'resultB' } },
        serviceC: { async test() { return 'resultC' } }
      },
      query: {
        testA: ['serviceA', 'test', {}],
        testB: ['serviceB', 'test', {}], 
    ...
    assert(serviceCalls.some(log => log.includes('[serviceA.test]')), 'Should log serviceA calls')
    assert(serviceCalls.some(log => log.includes('[serviceB.test]')), 'Should log serviceB calls') 
    assert(serviceCalls.some(log => log.includes('[serviceC.test]')), 'Should log serviceC calls')
    ...
    assert(foundColorCodes > 0, `Expected colored debug output, but found no ANSI color codes in logs: ${JSON.stringify(logCalls)}`)
  })
  ...
  it('should assign consistent colors to the same service across calls', async () => {
    ...
    await query({
      settings: { debug: true },
      services: {
        testService: { async method() { return 'result1' } }
      },
      query: {
    ...
    await query({
      settings: { debug: true },
      services: {
        testService: { async method() { return 'result2' } }
      },
      query: {
    ...
    const extractColor = (logs) => {
      const serviceLog = logs.find(log => log.includes('[testService.method]'))
    ...
    assert.strictEqual(firstColor, secondColor, 'Same service should use same color across calls')
  })
  ...
  it('should not use red color for services (reserved for errors)', async () => {
    await query({
      settings: { debug: true },
      services: {
        testService: { async method() { return 'result' } }
      },
      query: {
    ...
    const hasRedInDebugLogs = logCalls.some(log => 
      log.includes('[testService.method]') && log.includes(redColorCode)
    )
    
    assert(!hasRedInDebugLogs, 'Debug logs should not use red color (reserved for errors)')
  })
})